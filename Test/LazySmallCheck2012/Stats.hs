...
allSat pgrs n d xs = terms $ runSeries xs d
  where
    ...
    prop (ForAll   f xs) = (/= 0) `fmap2` allSat Nothing (n + 1) (f d) xs
    prop (Exists   f xs) = (== 0) `fmap2` allSat Nothing (n + 1) (f d) (fmap Not xs)
    refineWith _  es (C ct (Left (Expand (n', ps))) )
      | n == n' = C ct id <*> terms (es ps)
    refineWith pr _  (C m (Right True))  = unsafeAdd pgrs pr $ C m (Right pr)
    refineWith pr es (C m (Right False)) = unsafeAdd pgrs pr $ C m (Right 0)